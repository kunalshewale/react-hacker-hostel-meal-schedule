...
describe("Hacker Hostel <App />", () => {
  ...
  it("check if meals schedule is generated for valid entries", () => {
    ...
    wrapper.instance().handleHackerNameChange({
    ...
    expect(content).not.toContain(
      "Error! No menu generated for Johhny Alsodoes"
    );
  ...
  it("check if meals schedule is generated for valid entries in right order", () => {
    ...
    wrapper.instance().handleDateChange({
      target: { value: "2018-09-15 to 2018-09-15\n2018-09-15 to 2018-09-15" },
    });
    wrapper.find(Button).simulate("click");
    const content = wrapper.text();
    // TODO: validate the order as well
    expect(content).toContain(
      "Breakfast for John Doe on 2018-9-15Breakfast for Johhny Alsodoes on 2018-9-15Lunch for John Doe on 2018-9-15Lunch for Johhny Alsodoes on 2018-9-15Dinner for John Doe on 2018-9-15"
    );
    expect(content).not.toContain(
      "Error! No menu generated for Johhny Alsodoes"
    );
  ...
  it("check if meals schedule is generated when one of the date ranges is invalid", () => {
    ...
    wrapper.instance().handleDateChange({
      target: { value: "2017-09-15 to 2017-09-15\nInvalid date" },
    });
    wrapper.find(Button).simulate("click");
    const content = wrapper.text();
  ...
  it("check if no meals schedule is generated when both the date ranges are invalid", () => {
    ...
    wrapper.instance().handleDateChange({
      target: { value: "2017-09-17 to 2017-09-15\n2017-13-17 to 2017-09-1" },
    });
    wrapper.find(Button).simulate("click");
  ...
  it("check meals schedule when input is empty", () => {
    const wrapper = mount(<App />);
    wrapper.instance().handleHackerNameChange({ target: { value: "" } });
